...
export default function GuardianPhotoUploader({ guardianId, initialUrl, saveAction }: Props) {
  ...
  const bucket = useMemo(
    () => (process.env.NEXT_PUBLIC_GUARDIAN_PHOTOS_BUCKET ?? "guardian-photos").trim(),
    []
  );
  ...
  async function handleFile(file: File) {
    ...
    const path = `guardians/${guardianId}/${id}.${ext}`;
    ...
    <div className="space-y-3">
      ...
      >
        <div className="flex items-center justify-between gap-3">
          <div>
            <div className="text-sm font-semibold text-slate-900">Guardian photo</div>
