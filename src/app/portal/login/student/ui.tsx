...
export function StudentLoginForm({ redirectTo }: { redirectTo: string }) {
  ...
  const canSubmit = useMemo(() => {
    return admissionNumber.trim().length >= 2 && password.length >= 8;
  ...
  return (
    <div className="mx-auto max-w-md px-6 py-16">
      <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
        <div className="text-sm font-semibold text-slate-500">Student portal</div>
        <h1 className="mt-2 text-2xl font-semibold text-slate-900">Sign in</h1>
        <p className="mt-2 text-sm text-slate-600">Use your admission number and password.</p>
